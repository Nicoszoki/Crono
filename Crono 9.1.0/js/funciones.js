    ...
    Promise.all([asignaturasPromise, listaAsignaturasPromise])
        .then(([asignaturas, listaAsignaturas]) => {
            ...
            $(document).on('blur', '.editable-carga-horaria', function() {
                ...
                const cell = $(this); // Referencia a la celda que está siendo editada
                ...
                if (isNaN(nuevaCargaHoraria) || nuevaCargaHoraria <= 0) {
                    ...
                    cell.text(cell.attr('data-original')); // Restaurar el valor original si es inválido
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            cell.attr('data-original', nuevaCargaHoraria); // Actualizar el valor original
                        } else {
                            Swal.fire('Error', 'Hubo un problema al actualizar la carga horaria.', 'error');
                            cell.text(cell.attr('data-original')); // Restaurar el valor original si hay error
                        }
                    },
                    error: function() {
                        Swal.fire('Error', 'No se pudo actualizar la carga horaria.', 'error');
                        cell.text(cell.attr('data-original')); // Restaurar el valor original en caso de error
                    },
                });
            });
...
function descargarPDF(nombreGrupo, grado) {
    ...
    let tablaOriginal = document.getElementById('tablaHorarios');
    let tablaClonada = tablaOriginal.cloneNode(true); // Clonar la tabla original
    ...
    html2canvas(tablaClonada, {
        ...
        .then((canvas) => {
            let imgData = canvas.toDataURL('image/png'); // Convertir el canvas a una imagen
            let imgWidth = canvas.width; // Ancho de la imagen en píxeles
            let imgHeight = canvas.height; // Alto de la imagen en píxeles

            // Convertir píxeles a milímetros (jsPDF usa mm)
            let pxToMm = 0.264583; // 1 píxel = 0.264583 mm
            ...
            let doc = new jsPDF('landscape', 'mm', [pdfWidth, pdfHeight]);
...
function fusionarCeldasDuplicadas(tabla) {
    ...
    tabla.find('tbody tr').each(function() {
        ...
        $(this).find('td:not(.tr_first)').each(function() {
            let currentCell = $(this); // Celda actual
            let currentText = currentCell.text().trim(); // Texto sin espacios extras
...
function mostrarAsignaturas(asignaturas) {
    ...
    asignaturas.forEach(asignatura => {
        ...
        perfil += ` <div>
                        <button class='editarAsignatura' data-nombre='${asignatura.nombre}' style="background: none; border: none; cursor: pointer;">
                            <img src='img/iconos/Editar.png' alt='Editar' class='icono iconoGrupo' style="vertical-align: middle;">
                        </button>
                    </div>`;
        perfil += "</div>";
