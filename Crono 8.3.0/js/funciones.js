...
function descargarPDF(nombreGrupo, grado) {
    ...
    html2canvas(tablaClonada, { scale: 2 }) // Aumentar la escala para mayor calidad
        .then((canvas) => {
            ...
            const doc = new jsPDF('landscape', 'mm', [pdfHeight, pdfWidth]);
...
function fusionarCeldasDuplicadas(tabla) {
    ...
    tabla.find('tbody tr').each(function() {
        ...
        $(this).find('td:not(.tr_first)').each(function() {
            ...
            if (prevCell && prevCell.text().trim() === currentCell.text().trim() && currentCell.text().trim() !== '') {
